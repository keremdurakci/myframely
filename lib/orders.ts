...
export const FRAME_PRICE_USD_CENTS = 4000;

export type ShippingDestination = "CA" | "INTL";

export const SHIPPING_RATES_USD_CENTS: Record<ShippingDestination, number> = {
  CA: 3500,
  INTL: 1500,
};

export const SHIPPING_LABEL: Record<ShippingDestination, string> = {
  CA: "Canada",
  INTL: "US & Other Countries",
};

type AllowedCountry =
  Stripe.Checkout.SessionCreateParams.ShippingAddressCollection.AllowedCountry;

// Every country code Stripe Checkout can collect a shipping address for.
// "INTL" means literally that — everywhere Stripe ships, minus Canada,
// which gets its own (higher) flat rate below.
const ALL_SHIPPING_COUNTRIES: AllowedCountry[] = [
...
export function shippingCountriesFor(destination: ShippingDestination): AllowedCountry[] {
  if (destination === "CA") return ["CA"];
  return ALL_SHIPPING_COUNTRIES.filter((c) => c !== "CA");
}
