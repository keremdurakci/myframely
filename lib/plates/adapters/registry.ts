...
import { createOhioAdapter } from "./ohio.ts";
import { createNewHampshireAdapter } from "./newHampshire.ts";
import { createMissouriAdapter } from "./missouri.ts";
import { createKentuckyAdapter } from "./kentucky.ts";
import { createWyomingAdapter } from "./wyoming.ts";
...
const LIVE_ADAPTERS: Record<string, (config: StateConfig) => StatePlateAdapter> = {
  ...
  OH: () => createOhioAdapter(),
  NH: () => createNewHampshireAdapter(),
  MO: () => createMissouriAdapter(),
  KY: () => createKentuckyAdapter(),
  WY: () => createWyomingAdapter(),
};
